...
with st.sidebar:
    ...
    if st.session_state.manual_mode:
        ...
        if st.session_state.transition_progress < 1:
            ...
            # Interpolate values
            for div_id in CSI_DIVISIONS.keys():
                current = st.session_state.cost_adjustments[div_id]
                ...
                st.session_state.cost_adjustments[div_id] = new_value
        ...
        for div_id, div_info in CSI_DIVISIONS.items():
            current_adj = st.session_state.cost_adjustments.get(div_id, 0)
...
with tab1:
    ...
    def highlight_changes(val):
        if isinstance(val, str) and ('+' in val or '-' in val):
            if val.startswith('+') or (val.startswith('$+') and float(val.replace('$', '').replace(',', '').replace('+', '')) > 0):
                return 'color: #e74c3c'
            elif val.startswith('-') or (val.startswith('$-') and float(val.replace('$', '').replace(',', '').replace('-', '')) < 0):
                return 'color: #2ecc71'
        return ''
    
    styled_df = df_delta.style.applymap(highlight_changes, subset=['Change $', 'Change %'])
...
with col1:
    if st.button("📊 Export Data to CSV", key="export_csv"):
        export_data = []
        current_costs = costs.get("manual" if st.session_state.manual_mode else "combined", costs["baseline"])
